...
RUN mkdir -p /usr/src/geonode_dev
...
COPY . /usr/src/geonode_dev/
WORKDIR /usr/src/geonode_dev
...
RUN chmod +x /usr/src/geonode_dev/tasks.py \
    && chmod +x /usr/src/geonode_dev/entrypoint.sh
...
ENTRYPOINT /usr/src/geonode_dev/entrypoint.sh
